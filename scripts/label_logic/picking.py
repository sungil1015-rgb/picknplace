...
# 로봇 접근경로 충돌 검사(arm box / 컵 진입경로) ON/OFF. 로봇 컨트롤러가 좌표
# 받으면 경로 충돌을 자체 회피하므로 기본 OFF(중복·과거부 방지). 2026-06-05
ROBOT_PATH_COLLISION_CHECK = bool(CONFIG["gates"].get("robot_path_collision_check", False))
# single-cup 픽에서 노는 컵(dead cup B) 하강 충돌 검사 — robot_path_collision_check와
# 독립(로봇이 안 봐주는 노는 툴 자가충돌). 기본 ON. 2026-06-05
SINGLE_CUP_DEAD_CUP_CHECK = bool(CONFIG["gates"].get("single_cup_dead_cup_check", True))
SINGLE_CUP_DEAD_CUP_DESCENT_MM = float(CONFIG["gates"].get("single_cup_dead_cup_descent_mm", 150.0))
SINGLE_CUP_DEAD_CUP_PROTRUSION_MM = float(CONFIG["gates"].get("single_cup_dead_cup_protrusion_mm", 10.0))
GATE_S2_ARM_PROTRUSION_MM = float(CONFIG["gates"]["s2_arm_protrusion_mm"])
# 파지점 surface anchor를 예측 원기둥 표면 ±이 값(mm) 안 점으로 제한 (노이즈 배제). 2026-06-02
BOTTLE_GRASP_SURF_TOL_MM = 6.0
...
# 투명 대응(2026-06-01): 둘레 occlusion 검사에서 단일 픽셀 z는 투명 병의
# see-through 배경(먼 값)을 읽어 바깥 바닥을 가짜 occluder로 잡음(shot352 poly#0).
# inside/outside 모두 작은 윈도우의 near 분위수(=앞면 표면)로 비교해 회피.
# project_bottle_tilt_gt_calibration / docs 참조.
OCCLUSION_NEAR_WINDOW_PX = 5
OCCLUSION_NEAR_Q = 15.0
...
# 캡원 테두리 각도 커버리지 — confidence 곱셈 신호 (캡 검출된 병에만). 2026-06-05
_CRC = _BOT_CFG.get("cap_rim_coverage", {}) or {}
CAP_RIM_COV_ENABLE = bool(_CRC.get("enable", True))
CAP_RIM_COV_SLAB_MM = float(_CRC.get("slab_mm", 1.0))
CAP_RIM_COV_RIM_TOL_MM = float(_CRC.get("rim_tol_mm", 5.0))
CAP_RIM_COV_N_BINS = int(_CRC.get("n_bins", 36))
CAP_RIM_COV_EXPECTED_ARC_DEG = float(_CRC.get("expected_arc_deg", 180.0))
CAP_RIM_COV_MIN_SLAB_POINTS = int(_CRC.get("min_slab_points", 10))
CAP_RIM_COV_MIN_FACTOR = float(_CRC.get("min_factor", 0.6))
...
def _near_surface_depth(z_field, u, v, rad=OCCLUSION_NEAR_WINDOW_PX, q=OCCLUSION_NEAR_Q):
    """작은 윈도우 안 valid 깊이의 near 분위수(앞면 표면). 투명 see-through 배경
    (먼 값) 대신 카메라쪽 앞면을 골라 occlusion 가짜양성을 막음. 유효점 부족 시 nan."""
    H, W = z_field.shape
    u0, u1 = max(0, u - rad), min(W, u + rad + 1)
    v0, v1 = max(0, v - rad), min(H, v + rad + 1)
    patch = z_field[v0:v1, u0:u1]
    ...
    for i in range(len(pts)):
        ...
        for k in range(n_samples):
            ...
            if not (0 <= u_in < W and 0 <= v_in < H
                ...
                continue
            # 앞면(near) 표면끼리 비교 — 투명 병은 단일 픽셀 z_in이 see-through
            # 배경(먼 값)이라 바깥 바닥이 가짜 occluder로 잡힘. near 분위수로 회피.
            z_in = _near_surface_depth(z_field, u_in, v_in)
            z_out = _near_surface_depth(z_field, u_out, v_out)
            if not (np.isfinite(z_in) and np.isfinite(z_out)):
                continue
            n_total += 1
            # Outside FRONT is closer than inside FRONT by jump_mm → real occluder
...
def _compute_confidence(
    ...
    cap_rim_coverage: Optional[float] = None,
) -> tuple:
    """Combine 5 signals into confidence in [0, 1]. Returns (score, breakdown_dict).

    wall_margin 신호는 box_roi 폐기와 함께 제거(2026-05-13). valid_ratio 신호는
    가중치 0(무효)이라 제거(2026-06-05). 가중치 합은 나머지 5개로 1.0 유지.

    cap_rim_coverage(병, 캡 검출 시): 5신호 가중합을 구한 뒤 곱셈 페널티로 적용
    (가중치 체계는 안 건드림). factor = min_factor + (1-min_factor)*coverage.
    None이면(비병/캡 미검출/측정 불가) 페널티 없음.
    """
    ...
    score = float(np.clip(sum(SCORE_WEIGHTS[k] * v for k, v in breakdown.items()),
                          0.0, 1.0))
    # 캡원 테두리 커버리지 곱셈 페널티 (가중합 이후 — SCORE_WEIGHTS 영향 없음).
    if cap_rim_coverage is not None:
        cov = float(np.clip(cap_rim_coverage, 0.0, 1.0))
        factor = CAP_RIM_COV_MIN_FACTOR + (1.0 - CAP_RIM_COV_MIN_FACTOR) * cov
        breakdown["cap_rim_coverage"] = cov
        score = float(np.clip(score * factor, 0.0, 1.0))
...
def _check_class_and_depth(ctx: dict) -> Optional[PickResult]:
    ...
    grid = ctx["grid"]
    # 실측(raw) 깊이 보존 — occlusion 등 외부 가림 판정은 recovery로 채운 값이
    # 아니라 실제 센서 깊이로 해야 함(투명 fill이 둘레를 가짜 가림으로 만듦). 2026-06-01
    ctx["grid_raw"] = grid
...
def _cap_rim_angular_coverage(points, cap_ref, long_axis, e1, e2, radius_mm,
                              slab_mm, rim_tol_mm, n_bins, expected_arc_deg,
                              min_slab_points):
    """캡원 테두리 각도 커버리지 신호 ∈ [0,1] (측정 불가 시 None).

    캡 평면에서 축방향 ±slab_mm 슬랩을 잘라, 반경≈R(캡원 테두리, |radial-R|≤rim_tol)
    점들이 캡원 둘레(e1,e2 = 축에 수직 정규직교 기저)를 따라 얼마나 넓은 각도에
    분포하는지 측정. covered_arc / expected_arc 로 정규화.
      - 슬랩 점 < min_slab_points → None (측정 불가, 페널티 없음)
      - 테두리 점 0 → 0.0 (캡원 거의 관측 안 됨)
    """
    rel = np.asarray(points, np.float64) - np.asarray(cap_ref, np.float64)
    along = rel @ long_axis
    slab = np.abs(along) <= slab_mm
    if int(slab.sum()) < min_slab_points:
        return None
    perp = rel - np.outer(along, long_axis)
    radial = np.linalg.norm(perp, axis=1)
    rim = slab & (np.abs(radial - radius_mm) <= rim_tol_mm)
    if int(rim.sum()) == 0:
        return 0.0
    pr = perp[rim]
    ang = np.arctan2(pr @ e2, pr @ e1)            # [-pi, pi]
    bins = np.floor((ang + np.pi) / (2.0 * np.pi) * n_bins).astype(int)
    bins = np.clip(bins, 0, n_bins - 1)
    covered_arc_deg = int(np.unique(bins).size) / n_bins * 360.0
    return float(np.clip(covered_arc_deg / max(expected_arc_deg, 1e-6), 0.0, 1.0))
...
def _compute_bottle_pose(ctx: dict) -> Optional[PickResult]:
    ...
    if poly_aspect < DISC_ASPECT_MAX:
        # 위에서 본 disc 자세 — top-down single cup. disc가 잘 맞고 카메라를
        # 충분히 향할 때(cos_z≥0.7)만 채택. 실패/비스듬이면 측면 cylinder 경로로
        # fall-through. (2026-06-01: 예전엔 즉시 IMPOSSIBLE → 누운 중간 tilt 병
        # (aspect<2, 예: shot121)이 disc_tilt로 잘못 거부됐음.)
        disc = _fit_disc_plane(ctx["mask"], ctx["valid"], ctx["grid"])
        # 병 end-on(캡/바닥이 카메라 향함) disc는 반경이 본체(~30mm) 수준이어야
        # 한다. 본체 반경의 1.4배(≈42mm)를 넘으면 평면 fit이 병 끝면이 아니라 큰
        # 영역(이웃/바닥/기울어 길어진 몸통)을 잡은 오fit → disc 채택 말고 측면
        # cylinder 경로로 fall-through. (shot351 poly#1 r=82, shot352 poly#1 r=57이
        # 캡으로 오분류되던 케이스. RECOVERY_CYL_R_MAX=60은 너무 느슨했음.) 2026-06-02
        disc_r_max = (BOTTLE_KNOWN_RADIUS_MM or 30.0) * 1.4
        if (disc is not None and abs(float(disc["axis_dir"][2])) >= 0.7
                and float(disc["radius_mm"]) <= disc_r_max):
            ...
            return None
        # disc 채택 불가 → 측면 cylinder 경로로 진행 (fall-through)

    # ─── 측면 cylinder 자세 (길쭉한 폴리곤 또는 disc fall-through)
    # 축/tilt = 2d_locked cylinder fit (GT 13개로 검증, 2026-06-01). 기존엔 cap
    # 법선을 축으로 썼으나, 캡 검출이 평평한 끝면이 아니라 경사진 어깨를 잡아
    # tilt를 과대평가함(shot121 |z|=0.79 vs GT 0.29). fit_bottle_cylinder의
    # 2d_locked 축은 GT와 pickable/steep을 깨끗이 분리(|z| gap 0.535~0.620).
    # 캡은 보조(있으면 centroid/radius 신뢰 가산). 캡 없어도 fit으로 측면 픽 시도
    # — 캡 검출이 불신 소스라 hard 게이트에서 내림. 참조: project_bottle_tilt_gt_calibration
    try:
        from . import depth_recovery as _dr
        fit = _dr.fit_bottle_cylinder(
            ctx["grid"], ctx["mask"],
            known_radius_mm=BOTTLE_KNOWN_RADIUS_MM,
            polygon_2d=ctx["polygon_2d"],
    ...
    if not fit.get("fit_ok"):
        return _impossible(
            "bottle cylinder fit 실패 (fit_ok=False)  ← bottle.enable_cylinder_pose",
            cid, gates=["bottle_cylinder_fit"], n_valid_points=n)
    long_axis = np.asarray(fit["axis_dir"], dtype=np.float64)
    long_axis = long_axis / max(float(np.linalg.norm(long_axis)), 1e-12)

    # Cap detect (선택) — 있으면 centroid/radius/시각화에 사용, 없으면 fit으로 대체.
    cap = _detect_cap_plane(
        ctx["mask"], ctx["valid"], ctx["grid"], ctx["polygon_2d"],
    )
    cap_centroid = (np.asarray(cap["centroid"], dtype=np.float64)
                    if cap is not None
                    else np.asarray(fit["center"], dtype=np.float64))
    cap_fit_cos = (abs(float(np.asarray(cap["axis_dir"], dtype=np.float64) @ long_axis))
                   if cap is not None else float("nan"))
    ...
    if int((proj_axis > 0).sum()) < int((proj_axis < 0).sum()):
        long_axis = -long_axis
        proj_axis = -proj_axis
    # 3) 자세 게이트 — fit 축의 |z| (= 누운 정도). BOTTLE_AXIS_Z_MAX=0.57
    # ★ decouple: 자세(long_axis)는 Theil로 정확하지만 steep 병을 과소추정해 게이트를
    # 뚫음(228 GT56→34°). 게이트만 보수적 gate_axis_z(OLS/Theil 중 더 기운 값)로 판정.
    # (없으면 |axis_dir[2]| 폴백.) project_bottle_tilt_gt_calibration
    _gz = fit.get("gate_axis_z")
    cos_axis_z = float(_gz) if _gz is not None else abs(float(long_axis[2]))
    # ★ fit이 세움(upright)으로 감지(원형 마스크+법선 카메라 정면)했으면 측면 픽 금지.
    # 세움 디스크 fit이 실패하면 실루엣 폴백이 가짜 누운 축(axis_z~0.05)을 반환해
    # 세움 게이트를 뚫고 측면 픽이 나가던 버그(114 p0, 244 p0). 게이트로 보내면
    # 캡 보이는 세움병은 cap_top 흡착, 아니면 거부. 2026-06-04
    if fit.get("orientation") == "upright":
        cos_axis_z = max(cos_axis_z, 1.0)
    if cos_axis_z > BOTTLE_AXIS_Z_MAX:
        # ★ 세운/비스듬한 병: 측면 흡착 불가 → 캡 윗면(cap_top) 흡착 시도. 2026-06-04
        # 캡 평면이 확실히 검출되고(인라이어 충분+반경 sane) 카메라를 충분히 향할 때만.
        _cap_r_max = (BOTTLE_KNOWN_RADIUS_MM or 30.0) * 1.4
        if (cap is not None and int(cap.get("n_inliers", 0)) >= 150
                and float(cap["radius_mm"]) <= _cap_r_max
                and abs(float(np.asarray(cap["axis_dir"], np.float64)[2])) >= 0.7):
            normal_v = np.asarray(cap["axis_dir"], np.float64)
            normal_v = normal_v / max(float(np.linalg.norm(normal_v)), 1e-12)
            _cap_c = np.asarray(cap["centroid"], np.float64)
            if float(normal_v @ _cap_c) > 0:       # 카메라(원점) 쪽을 향하게
                normal_v = -normal_v
            _la = long_axis - float(long_axis @ normal_v) * normal_v
            _n_la = float(np.linalg.norm(_la))
            _la = _la / _n_la if _n_la > 1e-9 else np.array([1.0, 0.0, 0.0])
            _mid = np.cross(normal_v, _la)
            _mid = _mid / max(float(np.linalg.norm(_mid)), 1e-12)
            position = _cap_c + SAFETY_OFFSET_MM * normal_v
            ctx.update({
                "position": position, "normal": normal_v,
                "long_axis": _la, "mid_axis": _mid,
                "eigvals": np.array([0.0, 0.0, float(cap["radius_mm"])]),
                "initial_position": position.copy(),
                "bottle_fit": {
                    "method": "cap_top",               # 세운 병 캡 윗면 흡착
                    "radius_mm": float(cap["radius_mm"]),
                    "axis_z": cos_axis_z,
                    "cap_radius_mm": float(cap["radius_mm"]),
                    "cap_thickness_mm": float(cap["plane_thickness_mm"]),
                    "cap_inliers": int(cap["n_inliers"]),
                    "cap_centroid": _cap_c.tolist(),
                    "cap_inlier_yx": cap.get("inlier_yx"),
                    "axis_dir": np.asarray(fit["axis_dir"], np.float64).tolist(),
                },
            })
            return None
        return _impossible(
            f"bottle 세운 자세 (axis·Z={cos_axis_z:.2f} > {BOTTLE_AXIS_Z_MAX}) "
            f"— 측면 흡착 불가, 캡 윗면도 미검출  ← bottle.axis_z_max",
            cid, gates=["bottle_upright"], n_valid_points=n)
    # radius = cap ellipse minor (있으면, perspective 안 받은 진짜 반경) /
    # 없으면 fit 반경(known prior 30mm)
    # ★ 반경은 사전지식(known 30mm) 최우선 — cap 타원 오검출이 17.7mm 같은 반경을
    # 만들어 원기둥 크기·파지점(능선투영 radius*normal)이 병 안쪽으로 박힘. 2026-06-04
    if BOTTLE_KNOWN_RADIUS_MM is not None:
        radius = float(BOTTLE_KNOWN_RADIUS_MM)
    elif cap is not None:
        radius = float(cap["radius_mm"])
    else:
        radius = float(fit.get("radius_mm") or 30.0)
    ...
    # 실측 top surface mean (mask 안 카메라 쪽 5%).
    # ★ body_pts 반쪽 필터(proj_axis>0)는 캡이 검출됐을 때만 적용 — 그땐
    # cap_centroid가 진짜 캡 끝이라 +axis 쪽이 몸통이 맞다. 캡 미검출이면
    # cap_centroid=fit 중심(몸통 한가운데)이라 반쪽 필터가 무의미하고, 축이
    # z로 기울면(long_axis_z≠0) proj_axis 부호가 깊이와 상관돼 카메라쪽 윗면
    # 점을 통째로 버린다 → 흡착점이 병 바닥에 찍히던 버그(shot351 poly#2
    # pos_z 1023→984). 캡 미검출(현재 일반 경로)에선 전체 body 사용. 2026-06-02
    if cap is not None and int((proj_axis > 0).sum()) > 30:
        body_pts = points[proj_axis > 0]
    else:
        body_pts = points
    # ★ 파지점 후보를 예측 원기둥 표면(±tol) 위 점으로 제한 — 투명병 앞벽/반사
    # 노이즈가 원기둥보다 카메라쪽으로 튀어 상위 5%에 끼어 흡착점을 원기둥 위
    # 허공으로 끌어올리던 문제 해결. 충분히 남을 때만 적용(과소거 방지). 2026-06-02
    _rel = body_pts - cap_centroid
    _along = _rel @ long_axis
    _radial = np.linalg.norm(_rel - np.outer(_along, long_axis), axis=1)
    _on_surf = np.abs(_radial - radius) < BOTTLE_GRASP_SURF_TOL_MM
    if int(_on_surf.sum()) >= 20:
        body_pts = body_pts[_on_surf]
    ...
    # ★ 안전망: surface anchor가 원기둥 표면 밖(노이즈/inlier 부족 폴백)이면 표면(R)으로
    # 클램프 — 흡착점이 원기둥 위 허공으로 뜨는 것 원천 차단(예: 490 radial264→30). 2026-06-02
    # 기준축은 표시 원기둥(cap_end)과 동일하게 — cap 검출 centroid는 축에서 살짝
    # 벗어날 수 있어 표시 원기둥과 불일치(583 등). cap_end 없으면 cap_centroid 폴백.
    _cap_ref = np.asarray(fit.get("cap_end"), float) if fit.get("cap_end") else cap_centroid
    _ra = surface_anchor_real - _cap_ref
    _ra_along = float(_ra @ long_axis)
    _ra_perp = _ra - _ra_along * long_axis
    _ra_r = float(np.linalg.norm(_ra_perp))
    # ★능선(ridge) 투영: 흡착점의 반경 성분을 항상 "축→카메라" 방향(normal, 축에 수직)
    # 으로 R 고정. 데이터 평균(_ra_perp)은 see-through로 축 반대편까지 끌릴 수 있어
    # (469/468 등 normal과 178도 반대면 사례) 방향 자체를 믿을 수 없음. 흡착은 카메라쪽
    # 능선이 정답이고, along 성분은 아래 centroid 정렬이 결정 → 정보 손실 없음. 2026-06-04
    surface_anchor_real = _cap_ref + _ra_along * long_axis + radius * normal
    ...
    # 캡원 테두리 각도 커버리지 신호 (캡 검출됐을 때만 — _cap_ref가 진짜 캡끝).
    # {normal, mid_axis}는 long_axis에 수직인 정규직교 기저 → 캡원 평면 각도축.
    cap_rim_cov = None
    if cap is not None and CAP_RIM_COV_ENABLE:
        cap_rim_cov = _cap_rim_angular_coverage(
            points, _cap_ref, long_axis, normal, mid_axis, radius,
            slab_mm=CAP_RIM_COV_SLAB_MM, rim_tol_mm=CAP_RIM_COV_RIM_TOL_MM,
            n_bins=CAP_RIM_COV_N_BINS,
            expected_arc_deg=CAP_RIM_COV_EXPECTED_ARC_DEG,
            min_slab_points=CAP_RIM_COV_MIN_SLAB_POINTS)
    ctx["cap_rim_coverage"] = cap_rim_cov
    ctx.update({
        ...
        "bottle_fit": {
            "radius_mm": radius,
            "cap_rim_coverage": cap_rim_cov,
            "axis_z": cos_axis_z,
            "cap_detected": cap is not None,         # 2026-06-01: 캡은 선택
            "cap_radius_mm": float(cap["radius_mm"]) if cap is not None else None,
            "cap_thickness_mm": float(cap["plane_thickness_mm"]) if cap is not None else None,
            "cap_inliers": int(cap["n_inliers"]) if cap is not None else 0,
            "cap_normal_consistency": float(cap.get("normal_consistency", 0.0)) if cap is not None else None,
            "cap_valid_density": float(cap.get("valid_density", 1.0)) if cap is not None else None,
            "axis_src": "2d_locked_fit",          # 2026-06-01: 캡법선→fit 축
            "cap_fit_cos": float(cap_fit_cos),    # 캡법선↔fit 축 일치도 (진단, 캡 없으면 nan)
            "fit_radius_mm": float(fit.get("radius_mm", radius)),
            # 원기둥 모델은 캡끝 기준 전체길이(145mm)로 보정 — fit이 cap_end/cyl_model_end
            # 제공(투명병 cap 미검출 시 중앙앵커·overshoot 교정). 없으면 옛 폴리곤기반 폴백. 2026-06-02
            "cap_centroid": (fit.get("cap_end") or cap_centroid.tolist()),
            "cap_inlier_yx": cap.get("inlier_yx") if cap is not None else None,  # view_3d paint용
            # 캡(끝) 영역 + 거기서 표면 매칭된 픽셀 — 투명 몸통 아닌 잘 보존된 캡에서만
            # 매칭 시각화 (사용자 원칙). fit_bottle_cylinder가 채움. 2026-06-02
            "cap_region_yx": fit.get("cap_region_yx"),
            "cap_matched_yx": fit.get("cap_matched_yx"),
            "cyl_length": float(fit.get("cyl_model_length") or cyl_length),
            "cyl_end": (fit.get("cyl_model_end") or cyl_end.tolist()),    # 시각화용 (캡끝 145mm)
...
def _refine_pose_with_heatmap(ctx: dict) -> None:
    ...
    for _, cand_pos, cand_long in candidates[:50]:
        ...
        # 로봇 경로 충돌 OFF면 refine 후보 arm 충돌 필터도 skip (커버리지로만 채택).
        if ROBOT_PATH_COLLISION_CHECK:
...
def _check_structural(ctx: dict) -> Optional[PickResult]:
    ...
    # Occlusion perimeter
    try:
        occ_clear = _compute_occlusion_clear(ctx.get("grid_raw", ctx["grid"]),
                                              ctx["polygon_2d"], ctx["valid"])
    except Exception:
        occ_clear = 1.0
...
def _check_defer_gates_inner(ctx: dict, single_cup: bool) -> Optional[PickResult]:
    ...
    if single_cup:
        from .suction_score import (approach_box_check as _abc_q,
                                      approach_cylinder_check as _acc_q)
        ...
        for sign in (-1.0, +1.0):
            ...
            # 로봇 경로 충돌 OFF면 arm은 중립(0)으로 두고 neighbor만으로 방향 선택.
            if ROBOT_PATH_COLLISION_CHECK:
                ...
                arm_t = 0.0
                arm_n_out = 0
            # ★ 노는 컵(dead cup B) 하강 원기둥 충돌 — robot 플래그와 독립.
            #   cup B가 내려가는 경로(컵 반경 × 하강높이)에 self 아닌 장애물이
            #   solid로 솟아 있으면 cup B가 충돌. neighbor(평면 ±band)와 달리
            #   하강 "부피" 전체를 봄.
            if SINGLE_CUP_DEAD_CUP_CHECK:
                dc = _acc_q(grid, valid, cup_b_cand, normal, cup_radius,
                            t_min_mm=HEAD_CLEARANCE_MM,
                            t_max_mm=SINGLE_CUP_DEAD_CUP_DESCENT_MM,
                            self_mask=mask)
                dc_t = float(dc["max_t_mm"])
                dc_block = (dc["n_outside"] > 0
                            and dc_t > SINGLE_CUP_DEAD_CUP_PROTRUSION_MM)
            else:
                dc_t = 0.0
                dc_block = False
            ...
            # 우선순위: dead-cup 충돌 회피 최우선 → arm/neighbor ok 많을수록 →
            #           같은 등급이면 dc_t/arm_t/nb_n 작은 쪽. (오름차순 정렬, 작을수록 좋음)
            score = (int(dc_block), -(int(arm_ok) + int(nb_ok)), dc_t, arm_t, nb_n)
            candidates.append((score, sign, dc_block))
        candidates.sort(key=lambda c: c[0])
        single_sign = candidates[0][1]
        single_dc_block = candidates[0][2]
        cup_a = position
        cup_b = position + single_sign * SUCTION_CUP_PITCH_MM * long_axis
        n_a = int((np.linalg.norm(points - cup_a, axis=1) < cup_radius).sum())
        n_b = 0
        # 양 방향 모두 dead-cup 충돌이면 → DEFER (안전: 어느 쪽으로 놔도 cup B 부딪힘)
        if SINGLE_CUP_DEAD_CUP_CHECK and single_dc_block:
            return _defer(
                f"single-cup: 노는 컵(B) 하강 경로 충돌 — 양 방향 모두 장애물 "
                f"(> {SINGLE_CUP_DEAD_CUP_PROTRUSION_MM:.0f}mm 솟음)  "
                f"← gates.single_cup_dead_cup_*",
                cid, gates=["S2_dead_cup_collision"], n_valid_points=n,
                eigenvalues=eigvals, position_mm=position,
                long_axis=long_axis, mid_axis=mid_axis, normal=normal,
                cup_a_3d=cup_a, cup_b_3d=cup_b)
    else:
        cup_offset = SUCTION_CUP_PITCH_MM / 2.0
    ...
    arm_hl = (RECT_LONG_MM / 2.0) * ARM_BOX_SCALE
    arm_hs = (RECT_SHORT_MM / 2.0) * ARM_BOX_SCALE
    # 로봇 경로 충돌은 컨트롤러가 자체 회피 → OFF면 박스 계산 자체를 skip.
    # arm_max_protrusion_mm은 0.0으로 둠(다운스트림 정렬키/시각화가 .get로 읽어 무해).
    if ROBOT_PATH_COLLISION_CHECK:
        ...
        arm_chk = {"n_outside": 0, "max_t_mm": 0.0}
    ...
    # 좁아 catch 못 함.)
    # 로봇 경로 충돌은 컨트롤러가 자체 회피 → ROBOT_PATH_COLLISION_CHECK=False면 skip.
    if (ROBOT_PATH_COLLISION_CHECK
            and arm_chk["n_outside"] > 0
    ...
    # ─── S2 컵 진입 경로 — single-cup mode skip + 로봇 경로 충돌 OFF면 skip
    # (컵이 내려가는 실린더 경로의 장애물 = 로봇 컨트롤러가 자체 회피)
    for label, cup_3d in (("A", cup_a), ("B", cup_b)):
        if single_cup or not ROBOT_PATH_COLLISION_CHECK:
            break
...
def _compute_all_signals(ctx: dict, single_cup: bool = False) -> Optional[tuple]:
    ...
    if "occ_clear" in ctx:
        ...
        try:
            occ_clear = _compute_occlusion_clear(
                ctx.get("grid_raw", ctx["grid"]), ctx["polygon_2d"], ctx["valid"])
        except Exception:
            occ_clear = 1.0
        ctx["occ_clear"] = occ_clear
    ...
    confidence, breakdown = _compute_confidence(
        ...
        cap_rim_coverage=ctx.get("cap_rim_coverage"),
    )
...
def _inject_signals_into_fail(pr: PickResult, ctx: dict) -> PickResult:
    ...
    # 거부(DEFER/IMPOSSIBLE) 픽도 원기둥 복원(bottle_fit)을 보존 — _compute_bottle_pose가
    # ctx에 저장하지만 _defer는 안 실어줘서 빠졌었음(거부 시 3D에 원기둥 안 보임). 2026-06-02
    if pr.bottle_fit is None and ctx.get("bottle_fit") is not None:
        pr.bottle_fit = dict(ctx["bottle_fit"])
    res = _compute_all_signals(ctx)
    if res is None:
        return pr
    confidence, breakdown = res
